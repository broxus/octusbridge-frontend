...
export function formattedAmount(
    ...
    roundIfThousand: boolean = false,
): string {
    const parts = splitAmount(value, decimals)
    const isGreaterThanOrEqualThousand = new BigNumber(parts[0]).gte('1e3')
    const fractionalPart = new BigNumber(`0.${parts[1]}` || 0)
        .dp(4, BigNumber.ROUND_UP)
        .toFixed()
        .split('.')[1]
    return [
        formatDigits(parts[0]),
        // eslint-disable-next-line no-nested-ternary
        (roundIfThousand && isGreaterThanOrEqualThousand) ? undefined : (truncate ? fractionalPart : parts[1]),
    ].filter(e => e).join('.')
}
