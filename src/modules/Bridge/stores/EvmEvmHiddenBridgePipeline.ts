...
import type { FullContractState } from 'everscale-inpage-provider'
...
import type { EverscaleEvmPipelineState } from '@/modules/Bridge/stores/EverscaleEvmPipeline'
import { EvmEverscaleCreditPipeline } from '@/modules/Bridge/stores/EvmEverscaleCreditPipeline'
import type {
    EvmEverscaleCreditPipelineData,
    EvmEverscaleCreditPipelineState,
} from '@/modules/Bridge/stores/EvmEverscaleCreditPipeline'
import type {
...
export type EvmEvmHiddenBridgePipelineData = EvmEverscaleCreditPipelineData & {
...
export type EvmHiddenSwapTransferStoreState = EvmEverscaleCreditPipelineState & {
    secondEventState?: EverscaleEvmPipelineState['eventState'];
    secondPrepareState?: EverscaleEvmPipelineState['prepareState'];
    releaseState?: EverscaleEvmPipelineState['releaseState'];
}


export class EvmEvmHiddenBridgePipeline extends EvmEverscaleCreditPipeline<
    EvmEvmHiddenBridgePipelineData,
...
> {
    ...
    constructor(
        ...
        makeObservable<EvmEvmHiddenBridgePipeline, | 'debt'>(this, {
    ...
    public async resolve(): Promise<void> {
        ...
        try {
            ...
            this.setData({
                amount: new BigNumber(methodCall?.params[0].value || 0)
                ...
                tokenAmount: new BigNumber(methodCall?.params[5].value || 0).shiftedBy(-token.decimals).toFixed(),
            })
    ...
    public async release(): Promise<void> {
        ...
        const send = async (transactionType?: string) => {
            ...
            const vaultContract = new this.evmWallet.web3.eth.Contract(
                EthAbi.Vault,
                this.pipelineDefault.vaultAddress.toString(),
            )
        ...
        (async () => {
            ...
            if (cachedState === undefined || !cachedState?.isDeployed) {
                ...
                if (this.txHash !== undefined) {
                    try {
                        ...
                        } as EvmHiddenSwapTransferStoreState['transferState'])
                        ...
                        } as EvmHiddenSwapTransferStoreState['prepareState'])
                    }
                    catch (e) {
            ...
            if (this.creditProcessorState !== state) {
                this.setState('swapState', {
                    ...
                    isCanceling: false,
                    isProcessing: false,
                } as EvmHiddenSwapTransferStoreState['swapState'])
            }
            ...
            if (isCancelled || isProcessed) {
                ...
                const swapState: EvmHiddenSwapTransferStoreState['swapState'] = {
                    ...this.swapState,
                ...
                if (isProcessed) {
                    ...
                    this.setData('swapAmount', swapAmountNumber.toFixed())

                    if (isGoodBignumber(this.amount) && isGoodBignumber(swapAmountNumber)) {
                        this.setData('tokenAmount', new BigNumber(this.amount).minus(swapAmountNumber).toFixed())
                    }
            ...
            if (state >= CreditProcessorState.EventDeployInProgress) {
                ...
                if (this.eventState?.status !== 'confirmed' && this.eventState?.status !== 'rejected') {
                    ...
                    const eventState: EvmHiddenSwapTransferStoreState['eventState'] = {
    ...
    protected runReleaseUpdater(): void {
        ...
        (async () => {
            if (
                ...
                const vaultContract = evmVaultContract(this.pipelineDefault.vaultAddress.toString(), network.rpcUrl)
    ...
    protected async syncTransferFees(): Promise<void> {
        ...
        try {
            ...
            this.setData({
                minTransferFee: new BigNumber(results[0]?.expected_amount || 0)
                    .shiftedBy(-this.token.decimals)
                    .toFixed(),
                maxTransferFee: new BigNumber(results[1]?.expected_amount || 0)
                    .shiftedBy(-this.token.decimals)
                    .toFixed(),
            })
        }
    ...
    public get pipeline(): EvmEvmHiddenBridgePipelineData['pipeline'] {
        return this.pipelineCredit
    }

    public get pipelineCredit(): EvmEvmHiddenBridgePipelineData['pipelineCredit'] {
        return this.data.pipelineCredit
    }

    public get pipelineDefault(): EvmEvmHiddenBridgePipelineData['pipelineDefault'] {
        return this.data.pipelineDefault
    }

    public get contractAddress(): EvmEvmHiddenBridgePipelineData['contractAddress'] {
        return this.data.contractAddress
    }

    public get everscaleAddress(): EvmEvmHiddenBridgePipelineData['everscaleAddress'] {
        return this.data.everscaleAddress
    }

    public get maxTransferFee(): EvmEvmHiddenBridgePipelineData['maxTransferFee'] {
        return this.data.maxTransferFee
    }

    public get minTransferFee(): EvmEvmHiddenBridgePipelineData['minTransferFee'] {
        return this.data.minTransferFee
    }

    public get swapAmount(): EvmEvmHiddenBridgePipelineData['swapAmount'] {
        return this.data.swapAmount
    }

    public get tokenAmount(): EvmEvmHiddenBridgePipelineData['tokenAmount'] {
