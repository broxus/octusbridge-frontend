...
import { debug, error, findNetwork, throwException } from '@/utils'
import initBridge, { unpackDeposit, unpackTokenSettings } from '@/wasm/bridge'

export type SolanaTvmPipelineData = {
    amount: string
...
export type SolanaTvmPipelineState = {
    eventState?: {
...
export class SolanaTvmPipeline extends BaseStore<SolanaTvmPipelineData, SolanaTvmPipelineState> {
    ...
    public async resolve(): Promise<void> {
        ...
        try {
            ...
            const amount = new BigNumber(BigInt(deposit.event.data.amount).toString(10) || 0).shiftedBy(
                -(this.pipeline?.solanaTokenDecimals ?? 0),
            )
            ...
            const deriveEventAddress = (
                await solEverEventConfigurationContract(this.pipeline.solanaConfiguration)
    ...
    public async prepare(): Promise<void> {
        ...
        try {
            ...
            this.setState('prepareState', {
                ...this.prepareState,
                isDeploying: false,
            } as SolanaTvmPipelineState['prepareState'])
        }
    }
    ...
    protected runPrepareUpdater(): void {
        ...
        (async () => {
            ...
            const eventState: SolanaTvmPipelineState['eventState'] = {
    ...
    public get pipeline(): SolanaTvmPipelineData['pipeline'] {
        return this.data.pipeline
    }

    public get amount(): SolanaTvmPipelineData['amount'] {
        return this.data.amount
    }

    public get deriveEventAddress(): SolanaTvmPipelineData['deriveEventAddress'] {
        return this.data.deriveEventAddress
    }

    public get leftAddress(): SolanaTvmPipelineData['leftAddress'] {
        return this.data.leftAddress
    }

    public get rightAddress(): SolanaTvmPipelineData['rightAddress'] {
        return this.data.rightAddress
    }

    public get token(): SolanaTvmPipelineData['token'] {
        return this.data.token
    }

    public get eventState(): SolanaTvmPipelineState['eventState'] {
        return this.state.eventState
    }

    public get prepareState(): SolanaTvmPipelineState['prepareState'] {
        return this.state.prepareState
    }

    public get transferState(): SolanaTvmPipelineState['transferState'] {
