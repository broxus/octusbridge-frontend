...
export const RewardForm = observer(() => {
    ...
    const isOwner = transfer.leftAddress
        ? transfer.leftAddress.toLowerCase() === transfer.tvmWallet.address?.toLowerCase()
        : false
    ...
    const onSubmit = async (e: React.FormEvent<HTMLFormElement>): Promise<void> => {
        ...
        if (transfer.pipeline?.evmTokenDecimals !== undefined) {
            await transfer.changeBounty(
                BigNumber(localBounty)
                .shiftedBy(transfer.pipeline.evmTokenDecimals)
    ...
    React.useEffect(() => {
        if (transfer.pipeline?.evmTokenDecimals !== undefined) {
            setLocalBounty(
                transfer.bounty ? BigNumber(transfer.bounty).shiftedBy(-transfer.pipeline.evmTokenDecimals).toFixed() : '',
            )
        }
    }, [transfer.bounty, transfer.pipeline?.evmTokenDecimals])
