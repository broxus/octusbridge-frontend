...
export function ValidationRoundsInner(): JSX.Element | null {
    const intl = useIntl()
    ...
    const prevRound = rounds[0].roundNum - 1 >= 0 ? {
        ...
        content: (
            <Round
                link={`/relayers/round/${rounds[0].roundNum - 1}`}
                title={intl.formatMessage({
                    id: 'RELAYERS_CALENDAR_VALIDATION_ROUND',
                }, {
    ...
    const currentRound = {
        ...
        content: (
            <Round
                link={`/relayers/round/${rounds[0].roundNum}`}
                title={intl.formatMessage({
                    id: 'RELAYERS_CALENDAR_VALIDATION_ROUND',
                }, {
        ...
        content: (
            <Round
                link={getRoundStatus(rounds[1].startTime, rounds[1].endTime) !== 'waiting'
                    ? `/relayers/round/${rounds[0].roundNum + 1}`
                    : undefined}
                title={intl.formatMessage({
                    id: 'RELAYERS_CALENDAR_VALIDATION_ROUND',
                }, {
            ...
            <div className="rounds-calendar-scale rounds-calendar__names-short">
                <div className="rounds-calendar-scale__section">
                    <div className="rounds-calendar-round__title">
                        {intl.formatMessage({
                            id: 'RELAYERS_CALENDAR_VALIDATION_ROUNDS',
                        })}
                    </div>
                </div>
            </div>
